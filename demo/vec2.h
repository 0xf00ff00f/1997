...
struct vec2
{
    ...
    const vec2<T> operator+(const vec2<T> &v) const { return { x + v.x, y + v.y }; }
    ...
    const vec2<T> operator-() const { return { -x, -y }; }

    template <typename S>
    const vec2<T> operator*(S s) const
    {
        return { x * s, y * s };
    }
    ...
    const vec2<T> rotate(float ang) const
    {
...
inline const vec2<T> operator*(S s, const vec2<T> &v)
{
    return { v.x * s, v.y * s };
}
